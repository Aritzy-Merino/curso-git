...
setwd("/Users/aritzy/Documents/Rizika")
...
names(lista_metodos_1)[1] <- "metodo"

lista_metodos_1 <- lista_metodos_1 %>% 
  mutate(reclasificacion = ifelse(metodo == "desgarros musculares", "muerte asociada a posible tortura", 
                                  ifelse(metodo == "golpes", "muerte asociada a posible tortura", 
                                         ifelse(metodo == "rotura de huesos", "muerte asociada a posible tortura", 
                                                ifelse(metodo == "cortes", "muerte asociada a posible tortura", 
                                                       ifelse(metodo == "arrastrado con vehiculo", "muerte asociada a posible tortura", 
                                                                                                                       ifelse(metodo == "ingesta de producto químico", "muerte asociada a posible tortura", 
                                                                                                                              ifelse(metodo == "apuñalamiento", "muerte asociada a posible tortura", 
                                                                                                                                     ifelse(metodo == "aplastamiento", "muerte asociada a posible tortura", 
                                                                                                                                            ifelse(metodo == "ingesta de elemento cortante", "muerte asociada a posible tortura", 
                                                                                                                                                   ifelse(metodo == "quemaduras", "muerte asociada a posible tortura",
                                                                                                                                                   ifelse(metodo == "descargas eléctricas", "muerte asociada a posible tortura", 
                                                                                                                                                          ifelse(metodo == "amordazado", "inmovilización", 
                                                                                                                                                                 ifelse(metodo == "atado de pies y manos", "inmovilización",
                                                                                                                                                                        ifelse(metodo == "maniatado", "inmovilización",
                                                                                                                                                                               ifelse(metodo == "encobijado", "inmovilización",
                                                                                                                                                                                      ifelse(metodo == "ahorcamiento", "muerte por asfixia",
                                                                                                                                                                                             ifelse(metodo == "asfixia", "muerte por asfixia", 
                                                                                                                                                                                                    ifelse(metodo == "ahogamiento", "muerte por asfixia", 
                                                                                                                                                                                                           ifelse(metodo == "asfixia por estrangulamiento", "muerte por asfixia", 
                                                                                                                                                                                                                  ifelse(metodo == "baño con sustancia quí¬mica cáustica", "desaparición de cuerpo por métodos químicos", 
                                                                                                                                                                                                                         ifelse(metodo == "quemado (no es lo mismo que quemaduras)", "desaparición de cuerpo por métodos químicos", 
                                                                                                                                                                                                                                ifelse(metodo == "decapitado", "desmembramiento",
                                                                                                                                                                                                                                       ifelse(metodo == "mutilado", "desmembramiento", ifelse(metodo == "desfiguración", "desmembramiento", "muerte por arma de fuego")))))))))))))))))))))))))
  

write.csv(lista_metodos_1, file="lista_metodos_violencia.csv", fileEncoding ="UTF-8")

metodos <- metodos %>% 
  filter(metodo != "no especificado" & metodo != "descomposición" & metodo != "postura corporal incómoda"  & metodo != "buen estado de salud")
...
metodos_mun <- metodos %>% 
  select(3:6, 22) %>% 
  group_by(estado, reclasificacion) %>% 
  count()

names(metodos_mun)[5] <- "metodo"

metodos_mun <- metodos_mun %>% 
  spread(metodo, freq) %>% 
  group_by(estado, municipio)

metodos_mun[is.na(metodos_mun)]<-0

metodos_graf <- metodos %>% 
  select(21) %>% 
  group_by(metodo) %>% 
  count()

metodos_graf <- metodos_graf[order(metodos_graf$freq),]

graf_metodos <- ggplot(data = metodos_graf, aes(x = reorder(metodo, -freq), y = freq)) +
  geom_bar(stat = "identity") + 
  geom_text(data= metodos_graf,
            aes(x = metodo, y = freq,label= freq), position=position_dodge(width=0.9), vjust=-0.25) +
